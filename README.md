...
EA Web App이 영어가 아닌 언어로 설정되어 있으면 진화 이름을 `현재 언어명 (EA 공식 영어명)` 형식으로 함께 표시한다. 영어명은 진화 슬롯 ID와 EA의 공식 `en-US` 현지화 카탈로그를 연결한다. 최신 이름이 정적 카탈로그에 없으면 EA Web App을 영어로 열었을 때 실제 EA 진화 응답에서 같은 슬롯 ID의 영문 이름을 한 번 저장한다. 공식 값을 확보하지 못했을 때는 임의 번역을 만들지 않는다.
